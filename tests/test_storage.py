...
def test_storage_mapping_save_v1_creates_mapping_json(mock_bb_dir):
    """Test that storage_mapping_save_v1 creates proper mapping.json"""
    ...
    mapping_hash = bb.storage_mapping_save_v1(
...
def test_storage_mapping_save_v1_returns_hash(mock_bb_dir):
    """Test that storage_mapping_save_v1 returns the mapping hash"""
    func_hash = "abcd1234" + "0" * 56
    lang = "eng"
    docstring = "Test"
...
def test_storage_mapping_save_v1_deduplication(mock_bb_dir):
    ...
    bb.code_save_v1(
        func_hash2,
        normalize_code_for_test("def _bb_v_0(): return 42"),
        bb.code_create_metadata(),
    )

    # Save identical mappings for both
    mapping_hash1 = bb.storage_mapping_save_v1(
        func_hash1, lang, docstring, name_mapping, alias_mapping, comment
    )
    mapping_hash2 = bb.storage_mapping_save_v1(
...
def test_storage_mapping_save_v1_different_comments_different_hashes(mock_bb_dir):
    ...
    hash1 = bb.storage_mapping_save_v1(
        func_hash, lang, docstring, name_mapping, alias_mapping, "Formal"
    )
    hash2 = bb.storage_mapping_save_v1(
...
def test_v1_write_integration_full_structure(mock_bb_dir):
    ...
    eng_hash = bb.storage_mapping_save_v1(
        func_hash,
        "eng",
        "Double the input",
    ...
    fra_hash = bb.storage_mapping_save_v1(
        func_hash,
        "fra",
        "Doubler l'entrée",
...
def test_storage_mappings_list_v1_single_mapping(mock_bb_dir):
    """Test that storage_mappings_list_v1 returns single mapping correctly"""
    ...
    bb.storage_mapping_save_v1(
        func_hash, lang, docstring, name_mapping, alias_mapping, comment
    )

    # List mappings
    mappings = bb.storage_mappings_list_v1(func_hash, lang)
...
def test_storage_mappings_list_v1_multiple_mappings(mock_bb_dir):
    """Test that storage_mappings_list_v1 returns multiple mappings"""
    ...
    bb.storage_mapping_save_v1(
        func_hash, lang, "Doc 1", {"_bb_v_0": "func1"}, {}, "Formal"
    )
    bb.storage_mapping_save_v1(
        func_hash, lang, "Doc 2", {"_bb_v_0": "func2"}, {}, "Casual"
    )
...
def test_storage_mappings_list_v1_no_mappings(mock_bb_dir):
    """Test that storage_mappings_list_v1 returns empty list when no mappings exist"""
    ...
    mappings = bb.storage_mappings_list_v1(func_hash, "fra")

    # Should be empty
    assert len(mappings) == 0


def test_storage_mapping_load_v1_loads_correctly(mock_bb_dir):
    """Test that storage_mapping_load_v1 loads a specific mapping"""
    ...
    # Load the mapping
    loaded_doc, loaded_name, loaded_alias, loaded_comment = bb.storage_mapping_load_v1(
